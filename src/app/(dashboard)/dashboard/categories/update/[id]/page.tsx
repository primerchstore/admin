...
import {
  DescriptionInput,
  NameInput,
} from "@/app/(dashboard)/dashboard/categories/update/[id]/_components/form/text";
...
import { usePatchCategory } from "@/app/(dashboard)/dashboard/categories/_components/query";
import ParentInput from "@/app/(dashboard)/dashboard/categories/update/[id]/_components/form/parent";
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [parentId, setParentId] = useState<string | undefined | null>();
  ...
  useEffect(() => {
    if (isSuccess && data.result) {
      ...
      setParentId(data.result.parent?.id);
  ...
  if (!data || data.statusCode !== 200)
    return <EmptyData title="Category" icon={GridIcon} />;
